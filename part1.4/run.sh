...
HDFS_OUTPUT="/parking/analysis/vehicle_profiling"
LOG_DIR="logs"
LOG_FILE="${LOG_DIR}/task1.4.log"
...
header "CISC 5950  Lab 2  ·  Task 1.4  —  Vehicle Risk Profiling"
...
step "Launching MapReduce Job  [Task 1.4 — Vehicle Risk Profiling]"
info "Mapper  : mapper.py  (emits C→color and P→plate records)"
info "Reducer : reducer.py (color risk scores + repeat offenders)"
info "Input   : ${HDFS_INPUT}/"
info "Output  : ${HDFS_OUTPUT}/"
printf "\n"

$HADOOP jar "$STREAMING_JAR" \
    -D mapreduce.job.name="CISC5950-Lab2-Task1.4-VehicleProfiling" \
...
step "Results"
printf "\n"
$HDFS dfs -cat "${HDFS_OUTPUT}/part-"* 2>/dev/null | tee "${LOG_DIR}/task1.4_results.txt"

printf "\n"
info "Logs    : ${LOG_FILE}"
info "Results : ${LOG_DIR}/task1.4_results.txt"
